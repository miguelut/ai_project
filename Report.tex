...
major difference is that between the two is that, while breadth-first search
...
function \textit{g(n)} is non-decreasing.  Uniform-cost search has the
...
for this should be clear -- we want to find the best solution, and their may
...
complexity of depth first search must be given in terms of \textit{m}, the
maximum depth of any node.  With this in mind the time complexity of depth-first
search is $O(b^m)$, where \textit{m} can be much larger than the \textit{d} from
...
factor \textit{b}.  By its nature, the algorithm guarantees that all children
...
alleviates this issue.\cite{norvig}
